...
let mainWindow = null;
let psProcess  = null;
let isTransitioning = false;   // candado: evita start/stop concurrentes
let currentRunToken  = null;   // token unico por ejecucion, para limpieza por nombre
...
async function startScript() {
  // Candado: si ya hay una transicion start/stop en curso, ignorar.
  // Esto evita que doble-clicks o carreras de la UI lancen dos procesos.
  if (isTransitioning) return;
  isTransitioning = true;

  try {
    if (psProcess) await stopScriptAsync();
    // Limpieza de seguridad: matar cualquier PS1 huerfano de una ejecucion
    // previa que taskkill por PID no haya alcanzado a limpiar.
    await killOrphanedScripts();
    ...
    currentRunToken = tmpName;

  const PS1_SOURCE = "# ======================================================================\n#  Totalplay Club WiFi \u2014 Keep-alive (Electron Edition v31)\n#  Flujo IDENTICO al v23. JSON emitido con TODOS los campos siempre.\n# ======================================================================\n\nparam(\n    [string]$ConfigPath = \"\"\n)\n\n[System.Net.WebRequest]::DefaultWebProxy    = $null\n[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12\n\n# \u2500\u2500 CONFIG \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\nfunction Load-Config {\n    param([string]$Path)\n    $c = [pscustomobject]@{\n        portalBase         = \"https://clubwifi.totalplay.com.mx\"\n        accname            = \"\"\n        wlanparameter      = \"\"\n        ssidPreferido      = \"\"\n        intervaloKeepAlive = 240.0\n        intervaloReintento = 1.0\n        segsParaReconWifi  = 45\n        segsParaResetNic   = 70\n        hotspotAlInicio    = $true\n        tipoDispositivo          = \"DESKTOP\"\n        reconexionProactiva      = $false\n        intervaloProactivo       = 210.0\n        duracionDesconexion      = 1.2\n        macSpoofing              = $false\n        healthCheckAvanzado      = $true\n        fallbackSinDns           = $true\n        cooldownWifiConnect      = $true\n        duracionCooldownWifi     = 3.0\n    }\n    if ($Path -ne \"\" -and (Test-Path $Path)) {\n        try {\n            $j = Get-Content $Path -Raw | ConvertFrom-Json\n            if ($j.portalBase    -and $j.portalBase -ne \"\") { $c.portalBase         = $j.portalBase }\n            if ($j.accname)            { $c.accname            = $j.accname }\n            if ($j.wlanparameter)      { $c.wlanparameter      = $j.wlanparameter }\n            if ($j.ssidPreferido)      { $c.ssidPreferido      = $j.ssidPreferido }\n            if ($j.intervaloKeepAlive) { $c.intervaloKeepAlive = [double]$j.intervaloKeepAlive }\n            if ($j.intervaloReintento) { $c.intervaloReintento = [double]$j.intervaloReintento }\n            if ($j.segsParaReconWifi)  { $c.segsParaReconWifi  = [int]$j.segsParaReconWifi }\n            if ($j.segsParaResetNic)   { $c.segsParaResetNic   = [int]$j.segsParaResetNic }\n            if ($null -ne $j.hotspotAlInicio)          { $c.hotspotAlInicio          = [bool]$j.hotspotAlInicio }\n            if ($null -ne $j.reconexionProactiva)      { $c.reconexionProactiva      = [bool]$j.reconexionProactiva }\n            if ($j.intervaloProactivo)                 { $c.intervaloProactivo       = [double]$j.intervaloProactivo }\n            if ($j.duracionDesconexion)                { $c.duracionDesconexion      = [double]$j.duracionDesconexion }\n            if ($null -ne $j.macSpoofing)              { $c.macSpoofing              = [bool]$j.macSpoofing }\n            if ($null -ne $j.healthCheckAvanzado)      { $c.healthCheckAvanzado      = [bool]$j.healthCheckAvanzado }\n            if ($null -ne $j.fallbackSinDns)           { $c.fallbackSinDns           = [bool]$j.fallbackSinDns }\n            if ($null -ne $j.cooldownWifiConnect)      { $c.cooldownWifiConnect      = [bool]$j.cooldownWifiConnect }\n            if ($j.duracionCooldownWifi)               { $c.duracionCooldownWifi     = [double]$j.duracionCooldownWifi }\n            if ($j.tipoDispositivo) { $c.tipoDispositivo = $j.tipoDispositivo }\n        } catch { }\n    }\n    return $c\n}\n\n$cfg               = Load-Config -Path $ConfigPath\n$portalBase        = $cfg.portalBase\n$validarUrl        = \"$portalBase/ClubMovil/validar-ip\"\n$accname           = $cfg.accname\n$wlanparameter     = $cfg.wlanparameter\n$ssidPreferido     = $cfg.ssidPreferido\n$intervaloKeepAlive = $cfg.intervaloKeepAlive\n$intervaloReintento = $cfg.intervaloReintento\n$segsParaReconWifi  = $cfg.segsParaReconWifi\n$segsParaResetNic   = $cfg.segsParaResetNic\n$hotspotAlInicio         = $cfg.hotspotAlInicio\n$tipoDispositivo         = $cfg.tipoDispositivo\n$reconexionProactiva     = $cfg.reconexionProactiva\n$intervaloProactivo      = $cfg.intervaloProactivo\n$duracionDesconexion     = $cfg.duracionDesconexion\n$macSpoofing             = $cfg.macSpoofing\n$healthCheckAvanzado     = $cfg.healthCheckAvanzado\n$fallbackSinDns          = $cfg.fallbackSinDns\n$cooldownWifiConnect     = $cfg.cooldownWifiConnect\n$duracionCooldownWifi    = $cfg.duracionCooldownWifi\n\n$configMtime = if ($ConfigPath -ne \"\" -and (Test-Path $ConfigPath)) {\n    (Get-Item $ConfigPath).LastWriteTime\n} else { [DateTime]::MinValue }\n\n# \u2500\u2500 SEND-EVENT \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n# IMPORTANTE: siempre emite TODOS los campos para que el renderer\n# los pueda leer aunque sean string vacio o 0.\nfunction Send-Event {\n    param(\n        [string]$type,\n        [string]$msg       = \"\",\n        [string]$ip        = \"\",\n        [string]$ssid      = \"\",\n        [string]$jsession  = \"\",\n        [int]$httpStatus   = 0,\n        [bool]$online      = $false,\n        [int]$restante     = 0,\n        [bool]$reconexion  = $false\n    )\n    $time   = Get-Date -Format \"HH:mm:ss\"\n    $msgEsc = $msg     -replace '\"', \"'\"\n    $sipEsc = $ip      -replace '\"', \"'\"\n    $sssEsc = $ssid    -replace '\"', \"'\"\n    $sjsEsc = $jsession -replace '\"', \"'\"\n\n    # JSON con TODOS los campos \u2014 el renderer siempre puede leerlos\n    $json = '{\"type\":\"'   + $type        + '\"' +\n            ',\"msg\":\"'    + $msgEsc      + '\"' +\n            ',\"time\":\"'   + $time        + '\"' +\n            ',\"ip\":\"'     + $sipEsc      + '\"' +\n            ',\"ssid\":\"'   + $sssEsc      + '\"' +\n            ',\"jsessionid\":\"' + $sjsEsc  + '\"' +\n            ',\"httpStatus\":'  + $httpStatus   +\n            ',\"online\":'      + ($online.ToString().ToLower()) +\n            ',\"restante\":'    + $restante   +\n            ',\"reconexion\":' + ($reconexion.ToString().ToLower()) +\n            '}'\n\n    Write-Output $json\n    [Console]::Out.Flush()\n}\n\n# \u2500\u2500 INTERNET \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n$testUrl = \"http://www.msftconnecttest.com/connecttest.txt\"\n\n# FIX: WebClient sin timeout puede colgarse hasta 100s (default de .NET)\n# si la peticion se queda \"a medias\" en vez de fallar rapido. Usamos\n# Invoke-WebRequest con -TimeoutSec explicito para que Hay-Internet NUNCA\n# bloquee el bucle mas de unos segundos, sin importar el estado de la red.\nfunction Hay-Internet {\n    try {\n        $resp = Invoke-WebRequest -UseBasicParsing -Uri $testUrl -TimeoutSec 3\n        return ($resp.Content -match \"Microsoft Connect Test\")\n    } catch { return $false }\n}\n\n# \u2500\u2500 IP \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\nfunction Obtener-IP-Portal {\n    try {\n        $ips = Get-NetIPAddress -AddressFamily IPv4 -ErrorAction Stop |\n               Where-Object { $_.IPAddress -match \"^10\\.\" } |\n               Select-Object -ExpandProperty IPAddress\n        return ($ips | Select-Object -First 1)\n    } catch { return $null }\n}\n\n# \u2500\u2500 SSID \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\nfunction Get-SSIDActual {\n    try {\n        $info  = & netsh wlan show interfaces 2>$null\n        $linea = $info | Where-Object { $_ -match \"^\\s+SSID\\s+:\" } | Select-Object -First 1\n        if ($linea) { return ($linea -replace \"^\\s+SSID\\s+:\\s+\", \"\").Trim() }\n    } catch { }\n    return \"\"\n}\n\n# \u2500\u2500 AUTENTICAR \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n# \u2500\u2500 MAC SPOOFING \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\nfunction Get-MacOriginal {\n    try {\n        $adapter = Get-NetAdapter | Where-Object { $_.Status -eq \"Up\" -and $_.PhysicalMediaType -match \"802.11\" } | Select-Object -First 1\n        if ($adapter) { return $adapter.MacAddress }\n    } catch { }\n    return \"\"\n}\n\nfunction Set-MacAdapter {\n    param([string]$AdapterName, [string]$NewMac)\n    try {\n        $regKey = \"HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4D36E972-E325-11CE-BFC1-08002BE10318}\"\n        $subkeys = Get-ChildItem $regKey -ErrorAction Stop\n        foreach ($k in $subkeys) {\n            $driverDesc = (Get-ItemProperty $k.PSPath -Name \"DriverDesc\" -ErrorAction SilentlyContinue).DriverDesc\n            if ($driverDesc -and (Get-NetAdapter -Name $AdapterName -ErrorAction SilentlyContinue).InterfaceDescription -eq $driverDesc) {\n                Set-ItemProperty $k.PSPath -Name \"NetworkAddress\" -Value ($NewMac -replace \"-\",\"\") -Type String\n                Restart-NetAdapter -Name $AdapterName -Confirm:$false\n                Start-Sleep -Seconds 3\n                return $true\n            }\n        }\n    } catch { }\n    return $false\n}\n\nfunction Get-RandomMac {\n    # Generar MAC aleatoria con bit unicast/locally-administered\n    $bytes = 1..6 | ForEach-Object { Get-Random -Minimum 0 -Maximum 255 }\n    $bytes[0] = ($bytes[0] -band 0xFE) -bor 0x02  # locally administered, unicast\n    return ($bytes | ForEach-Object { $_.ToString(\"X2\") }) -join \"-\"\n}\n\n# \u2500\u2500 HEALTH CHECK AVANZADO (del Premium) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\nfunction Resolve-PortalHost {\n    try {\n        $u = [uri]$portalBase\n        return $u.Host\n    } catch { }\n    return \"clubwifi.totalplay.com.mx\"\n}\n\nfunction Test-HostTcpQuick {\n    param([string]$HostOrIp, [int]$Port = 443, [int]$TimeoutMs = 1200)\n    try {\n        $client = New-Object System.Net.Sockets.TcpClient\n        $iar    = $client.BeginConnect($HostOrIp, $Port, $null, $null)\n        if (-not $iar.AsyncWaitHandle.WaitOne($TimeoutMs, $false)) { $client.Close(); return $false }\n        $client.EndConnect($iar) | Out-Null; $client.Close(); return $true\n    } catch { return $false }\n}\n\nfunction Get-DefaultGateway {\n    try {\n        return (Get-NetRoute -AddressFamily IPv4 -DestinationPrefix \"0.0.0.0/0\" -ErrorAction Stop |\n                Sort-Object -Property RouteMetric | Select-Object -First 1 -ExpandProperty NextHop)\n    } catch { return $null }\n}\n\n# \u2500\u2500 DNS REFORZADO \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n# El DNS que entrega el AP publico a veces falla o es asimetrico: unos\n# dominios resuelven y otros no (se vio en pruebas reales que\n# clubwifi.totalplay.com.mx fallaba mientras msftconnecttest.com seguia\n# respondiendo). Agregar DNS publico como respaldo en el adaptador WiFi\n# soluciona esto para toda la PC, no solo para el script.\n$script:_dnsReforzadoAplicado = $false\nfunction Set-DnsReforzado {\n    try {\n        $adapter = Get-NetAdapter | Where-Object { $_.Status -eq \"Up\" -and $_.PhysicalMediaType -match \"802.11\" } | Select-Object -First 1\n        if ($adapter) {\n            Set-DnsClientServerAddress -InterfaceIndex $adapter.ifIndex -ServerAddresses (\"8.8.8.8\",\"1.1.1.1\") -ErrorAction Stop\n            $script:_dnsReforzadoAplicado = $true\n            return $true\n        }\n    } catch { }\n    return $false\n}\n\nfunction Test-DnsResuelve {\n    param([string]$HostName, [int]$TimeoutMs = 1200)\n    try {\n        $iar = [System.Net.Dns]::BeginGetHostAddresses($HostName, $null, $null)\n        if (-not $iar.AsyncWaitHandle.WaitOne($TimeoutMs, $false)) { return $false }\n        [System.Net.Dns]::EndGetHostAddresses($iar) | Out-Null\n        return $true\n    } catch { return $false }\n}\n\nfunction Invoke-RecuperacionDnsRapida {\n    Send-Event -type \"warn\" -msg \"DNS fallando -- aplicando DNS publico y refrescando...\"\n    try { & ipconfig /flushdns 2>$null | Out-Null } catch { }\n    Set-DnsReforzado | Out-Null\n    Start-Sleep -Milliseconds 400\n}\n\n# \u2500\u2500 OBTENER BSSID DEL AP (automatico) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\nfunction Get-BSSID {\n    try {\n        $info  = & netsh wlan show interfaces 2>$null\n        $linea = $info | Where-Object { $_ -match \"^\\s+BSSID\\s+:\" } | Select-Object -First 1\n        if ($linea) { return ($linea -replace \"^\\s+BSSID\\s+:\\s+\", \"\").Trim() }\n    } catch { }\n    return \"\"\n}\n\nfunction Autenticar ($ip) {\n    # Detectar BSSID y derivar parametros\n    $bssidDetectado = Get-BSSID\n    $wlanparamUsado = if ($bssidDetectado -ne \"\") { $bssidDetectado.ToLower() } else { $wlanparameter }\n    $accnameUsado   = if ($bssidDetectado -ne \"\") { $bssidDetectado.ToUpper() -replace \":\", \"-\" } else { $accname }\n\n    $typeParam = if ($tipoDispositivo -eq \"ANDROID\") { \"ANDROID\" } else { \"DESKTOP\" }\n    $uaStr     = if ($tipoDispositivo -eq \"ANDROID\") {\n        \"Mozilla/5.0 (Linux; Android 13; SM-G981B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Mobile Safari/537.36\"\n    } else {\n        \"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36 Edg/146.0.0.0\"\n    }\n\n    $inicioUrl = \"$portalBase/ClubMovil/inicio?wlanuserip=$ip&wlanacname=&wlanparameter=$wlanparamUsado&accname=$accnameUsado&type=$typeParam&webView=false\"\n    $body      = \"{\\`\"ip\\`\":\\`\"$ip\\`\",\\`\"nombreAcc\\`\":\\`\"$accnameUsado\\`\",\\`\"mac\\`\":\\`\"$wlanparamUsado\\`\",\\`\"tipo\\`\":\\`\"$typeParam\\`\"}\"\n\n    $session = New-Object Microsoft.PowerShell.Commands.WebRequestSession\n    $session.UserAgent = $uaStr\n\n    # ESTRATEGIA: primero intentar POST directo (sin GET)\n    # El servidor acepta POST solo con IP+BSSID cuando la sesion esta activa.\n    # El GET falla con 500 cuando ya hay sesion activa \u2014 no es necesario.\n    try {\n        $r2 = Invoke-WebRequest -UseBasicParsing -Uri $validarUrl `\n            -Method POST -WebSession $session `\n            -Headers @{\n                \"Accept\"          = \"*/*\"\n                \"Accept-Language\" = \"es-MX,es;q=0.9\"\n                \"Origin\"          = $portalBase\n                \"Referer\"         = $inicioUrl\n                \"Sec-Fetch-Dest\"  = \"empty\"\n                \"Sec-Fetch-Mode\"  = \"cors\"\n                \"Sec-Fetch-Site\"  = \"same-origin\"\n            } `\n            -ContentType \"application/json\" -Body $body -TimeoutSec 6\n        return [int]$r2.StatusCode\n    } catch {\n        # POST fallo \u2014 sesion realmente muerta, hacer flujo completo GET + POST\n        Send-Event -type \"gray\" -msg \"POST directo fallo, intentando GET + POST completo...\"\n        $session2 = New-Object Microsoft.PowerShell.Commands.WebRequestSession\n        $session2.UserAgent = $uaStr\n        try {\n            Invoke-WebRequest -UseBasicParsing -Uri $inicioUrl `\n                -WebSession $session2 `\n                -Headers @{\n                    \"Accept\"          = \"text/html,application/xhtml+xml,*/*;q=0.8\"\n                    \"Accept-Language\" = \"es-MX,es;q=0.9\"\n                } -TimeoutSec 6 | Out-Null\n        } catch { <# GET puede dar 500 si sesion activa, ignorar #> }\n\n        $r3 = Invoke-WebRequest -UseBasicParsing -Uri $validarUrl `\n            -Method POST -WebSession $session2 `\n            -Headers @{\n                \"Accept\"          = \"*/*\"\n                \"Accept-Language\" = \"es-MX,es;q=0.9\"\n                \"Origin\"          = $portalBase\n                \"Referer\"         = $inicioUrl\n                \"Sec-Fetch-Dest\"  = \"empty\"\n                \"Sec-Fetch-Mode\"  = \"cors\"\n                \"Sec-Fetch-Site\"  = \"same-origin\"\n            } `\n            -ContentType \"application/json\" -Body $body -TimeoutSec 6\n        return [int]$r3.StatusCode\n    }\n}\n# \u2500\u2500 REINICIAR ADAPTADOR \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\nfunction Reset-Adaptador {\n    Send-Event -type \"warn\" -msg \"${segsParaResetNic}s sin internet -- reiniciando adaptador de red...\"\n    try {\n        # FIX: el filtro anterior exigia Status -eq \"Up\", pero justo\n        # cuando el adaptador esta atascado (p.ej. en \"Conectando...\"\n        # tras un livelock de netsh) su Status YA NO es \"Up\" -- por eso\n        # esta funcion fallaba con \"No se encontro adaptador activo\"\n        # exactamente cuando mas se necesitaba resetearlo.\n        # Ahora: primero intenta encontrar uno \"Up\" (caso normal), y si\n        # no hay ninguno, cae a CUALQUIER adaptador fisico WiFi sin\n        # importar su estado actual (Disconnected/Connecting/etc).\n        $adapter = Get-NetAdapter | Where-Object {\n            $_.Status -eq \"Up\" -and $_.Name -notmatch \"Loopback|Virtual|Bluetooth\"\n        } | Select-Object -First 1\n\n        if (-not $adapter) {\n            $adapter = Get-NetAdapter | Where-Object {\n                ($_.PhysicalMediaType -match \"802.11\" -or $_.Name -match \"Wi-?Fi|Wireless|WLAN\") -and\n                $_.Name -notmatch \"Loopback|Virtual|Bluetooth\"\n            } | Select-Object -First 1\n        }\n\n        if ($adapter) {\n            Disable-NetAdapter -Name $adapter.Name -Confirm:$false -ErrorAction Stop\n            Start-Sleep -Seconds 3\n            Enable-NetAdapter -Name $adapter.Name -Confirm:$false -ErrorAction Stop\n            Start-Sleep -Seconds 5\n            Send-Event -type \"info\" -msg \"Adaptador reiniciado: $($adapter.Name)\"\n            if ($fallbackSinDns) { Set-DnsReforzado | Out-Null }\n        } else {\n            Send-Event -type \"warn\" -msg \"No se encontro adaptador activo para reiniciar\"\n        }\n    } catch {\n        Send-Event -type \"warn\" -msg \"No se pudo reiniciar adaptador (requiere admin)\"\n    }\n}\n\nfunction Sleep-Reintento { param([double]$segundos) Start-Sleep -Milliseconds ([math]::Round([math]::Max(0.1, $segundos) * 1000)) }\n\n# \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\n#  INICIO\n# \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\nSend-Event -type \"status\" -msg \"iniciado\" -ssid $ssidPreferido\nSend-Event -type \"info\"   -msg \"Keep-alive: ${intervaloKeepAlive}s | Reintento: ${intervaloReintento}s | WiFi@${segsParaReconWifi}s | NIC@${segsParaResetNic}s\"\n\nif ($hotspotAlInicio) {\n    & netsh wlan start hostednetwork 2>$null | Out-Null\n    Send-Event -type \"info\" -msg \"Hotspot activado al inicio\"\n}\n\nif ($fallbackSinDns) {\n    if (Set-DnsReforzado) {\n        Send-Event -type \"gray\" -msg \"DNS publico (8.8.8.8 / 1.1.1.1) aplicado como respaldo\"\n    }\n}\n\n# \u2500\u2500 HEARTBEAT DE TRAFICO CONTINUO \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n# El cliente oficial del portal (capturado en su JS: \"ejecutarCada10sPor5m\")\n# solo renueva /validar-ip los primeros 5 minutos. Despues de eso, lo que\n# probablemente mantiene la sesion viva es la PRESENCIA DE TRAFICO real,\n# no llamadas al endpoint de auth. Aunque ya renovamos mucho mas rapido\n# que el cliente oficial, puede haber micro-huecos de silencio real entre\n# un ciclo del bucle principal y el siguiente. Este hilo de fondo, TOTALMENTE\n# independiente del bucle de autenticacion, manda pings diminutos al\n# gateway sin parar, para que nunca haya un hueco real de trafico cero.\n# Corre en un runspace (hilo del MISMO proceso) \u2014 si el proceso principal\n# muere (taskkill), este hilo muere con el automaticamente, sin huerfanos.\n$script:heartbeatPS = $null\nfunction Start-HeartbeatTrafico {\n    $gw = Get-DefaultGateway\n    if (-not $gw) { return }\n    try {\n        $rs = [runspacefactory]::CreateRunspace()\n        $rs.Open()\n        $ps = [powershell]::Create()\n        $ps.Runspace = $rs\n        [void]$ps.AddScript({\n            param($gatewayIp)\n            $pingObj = New-Object System.Net.NetworkInformation.Ping\n            while ($true) {\n                try { $pingObj.Send($gatewayIp, 400) | Out-Null } catch { }\n                Start-Sleep -Milliseconds 200\n            }\n        }).AddArgument($gw)\n        [void]$ps.BeginInvoke()\n        $script:heartbeatPS = $ps\n        Send-Event -type \"gray\" -msg \"Heartbeat de trafico continuo activo (gateway: $gw)\"\n    } catch { }\n}\nStart-HeartbeatTrafico\n\n$ultimaRenovacion       = [DateTime]::MinValue\n$ultimaReconexionActiva = [DateTime]::Now\n$ultimaVerifDns         = [DateTime]::MinValue\n# MinValue para que el PRIMER intento de reconexion WiFi (rama \"Sin IP\")\n# dispare de inmediato, sin esperar el cooldown de 3s.\n$ultimoIntentoWifiConnect = [DateTime]::MinValue\n$macOriginal            = Get-MacOriginal\n\n# $veniaDeCaida: true desde el primer ciclo sin internet hasta auth exitosa\n# Se limpia SOLO en el try-catch cuando auth es exitosa y fueCalda=true\n$veniaDeCaida = $false\n\n# Detectar transici\u00f3n offline -> online (conectividad) para contar reconexi\u00f3n\n# aunque el portal/auth tarde o falle (500/DNS/timeout).\n$internetPrev = $true\n\n# Escalada \u2014 adicionales al v23, no tocan el flujo original\n$inicioCaida     = $null   # DateTime del inicio de la caida actual\n$wifiReconectado = $false  # nivel 2 ya ejecutado en esta caida\n$nicReiniciada   = $false  # nivel 3 ya ejecutado en esta caida\n\n# \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\n#  BUCLE PRINCIPAL \u2014 estructura del v23\n# \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\nwhile ($true) {\n\n    # \u2500\u2500 Recarga en caliente de config \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n    if ($ConfigPath -ne \"\" -and (Test-Path $ConfigPath)) {\n        try {\n            $mtime = (Get-Item $ConfigPath).LastWriteTime\n            if ($mtime -ne $configMtime) {\n                $configMtime        = $mtime\n                $cfg                = Load-Config -Path $ConfigPath\n                $portalBase         = $cfg.portalBase\n                $validarUrl         = \"$portalBase/ClubMovil/validar-ip\"\n                $accname            = $cfg.accname\n                $wlanparameter      = $cfg.wlanparameter\n                $ssidPreferido      = $cfg.ssidPreferido\n                $intervaloKeepAlive = $cfg.intervaloKeepAlive\n                $intervaloReintento = $cfg.intervaloReintento\n                $segsParaReconWifi  = $cfg.segsParaReconWifi\n                $segsParaResetNic   = $cfg.segsParaResetNic\n                $hotspotAlInicio    = $cfg.hotspotAlInicio\n                $tipoDispositivo    = $cfg.tipoDispositivo\n                $reconexionProactiva = $cfg.reconexionProactiva\n                $intervaloProactivo  = $cfg.intervaloProactivo\n                $duracionDesconexion = $cfg.duracionDesconexion\n                $macSpoofing         = $cfg.macSpoofing\n                $healthCheckAvanzado = $cfg.healthCheckAvanzado\n                $fallbackSinDns      = $cfg.fallbackSinDns\n                $cooldownWifiConnect = $cfg.cooldownWifiConnect\n                $duracionCooldownWifi = $cfg.duracionCooldownWifi\n                Send-Event -type \"info\" -msg \"Config recargada: KA:${intervaloKeepAlive}s | RI:${intervaloReintento}s | WiFi@${segsParaReconWifi}s | NIC@${segsParaResetNic}s\"\n            }\n        } catch { }\n    }\n\n    # \u2500\u2500 RECONEXION PROACTIVA (configurable) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n    if ($reconexionProactiva -and $ssidPreferido -ne \"\") {\n        $segsDesdeReconexion = ([DateTime]::Now - $ultimaReconexionActiva).TotalSeconds\n        if ($segsDesdeReconexion -ge [double]$intervaloProactivo) {\n            $durMs = [int]([math]::Max(0.1, [double]$duracionDesconexion) * 1000)\n\n            # MAC Spoofing opcional: cambiar MAC antes de reconectar\n            if ($macSpoofing) {\n                $adapterInfo = Get-NetAdapter | Where-Object { $_.Status -eq \"Up\" -and $_.PhysicalMediaType -match \"802.11\" } | Select-Object -First 1\n                if ($adapterInfo) {\n                    $newMac = Get-RandomMac\n                    Send-Event -type \"info\" -msg \"MAC Spoofing: $($adapterInfo.MacAddress) -> $newMac\"\n                    Set-MacAdapter -AdapterName $adapterInfo.Name -NewMac $newMac | Out-Null\n                }\n            }\n\n            Send-Event -type \"info\" -msg \"Reconexion proactiva (${segsDesdeReconexion}s / cada ${intervaloProactivo}s) -- reiniciando sesion...\"\n            & netsh wlan disconnect 2>$null | Out-Null\n            Start-Sleep -Milliseconds $durMs\n            & netsh wlan connect name=\"$ssidPreferido\" 2>$null | Out-Null\n            Start-Sleep -Milliseconds ([math]::Max(500, $durMs))\n            $ultimaReconexionActiva = [DateTime]::Now\n            $ultimaRenovacion       = [DateTime]::MinValue\n        }\n    }\n\n    # \u2500\u2500 HEALTH CHECK AVANZADO (del Premium) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n\n    $internet    = Hay-Internet\n\n    # Health check avanzado: verificar TCP al portal si MSFT falla\n    if ((-not $internet) -and $healthCheckAvanzado) {\n        $portalHost  = Resolve-PortalHost\n        $portalTcpOk = Test-HostTcpQuick -HostOrIp $portalHost -Port 443 -TimeoutMs 1500\n        if ($portalTcpOk) {\n            $internet = $true\n            Send-Event -type \"gray\" -msg \"Health-check: portal alcanzable (MSFT test fallo)\"\n        } elseif ($fallbackSinDns) {\n            $gw = Get-DefaultGateway\n            if ($gw -and (Test-HostTcpQuick -HostOrIp $gw -Port 53 -TimeoutMs 900)) {\n                Send-Event -type \"warn\" -msg \"Fallback sin DNS: gateway OK -- aplicando recuperacion rapida...\"\n                Invoke-RecuperacionDnsRapida\n            }\n        }\n    }\n\n    # \u2500\u2500 DETECCION DE FALSO POSITIVO (con throttle) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n    # msftconnecttest.com puede seguir respondiendo (algunos APs lo dejan\n    # pasar aunque el resto de internet este degradado) mientras el DNS\n    # real del portal esta caido. Verificamos DNS del portal por separado.\n    # THROTTLE: antes corria en CADA ciclo (varias veces por segundo con\n    # keep-alive bajo), generando trafico DNS de fondo innecesario que\n    # podia sumar jitter. Ahora solo cada 20s \u2014 mas que suficiente para\n    # detectar el problema sin generar ruido constante.\n    if ($internet -and $fallbackSinDns) {\n        $segsDesdeVerifDns = ([DateTime]::Now - $ultimaVerifDns).TotalSeconds\n        if ($segsDesdeVerifDns -ge 20.0) {\n            $ultimaVerifDns = [DateTime]::Now\n            $portalHostFP = Resolve-PortalHost\n            if (-not (Test-DnsResuelve -HostName $portalHostFP -TimeoutMs 900)) {\n                Send-Event -type \"warn\" -msg \"DNS del portal no resuelve aunque hay conexion basica -- corrigiendo...\"\n                Invoke-RecuperacionDnsRapida\n            }\n        }\n    }\n\n    $segsDesde   = ([DateTime]::Now - $ultimaRenovacion).TotalSeconds\n    $debeRenovar = $segsDesde -ge [double]$intervaloKeepAlive\n\n    # \u2500\u2500 Transici\u00f3n: internet volvi\u00f3 \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n    # Si ven\u00edamos de una ca\u00edda (veniaDeCaida/inicioCaida) y ahora hay internet,\n    # emitir evento de recuperaci\u00f3n ya (para contadores/AVG en UI).\n    if ((-not $internetPrev) -and $internet) {\n        if ($veniaDeCaida -or $inicioCaida) {\n            $ssidActual = Get-SSIDActual\n            $ssidToSend = if ($ssidActual) { $ssidActual } else { $ssidPreferido }\n            Send-Event -type \"ok\" `\n                -msg \"Internet restaurado -- re-autenticando...\" `\n                -online $true -reconexion $true `\n                -ssid $ssidToSend\n        }\n        # limpiar escalada al recuperar conectividad\n        $inicioCaida     = $null\n        $wifiReconectado = $false\n        $nicReiniciada   = $false\n    }\n    $internetPrev = $internet\n\n    # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\n    # CASO 1 \u2014 Online y NO toca renovar (v23: if $internet -and -not $debeRenovar)\n    # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\n    if ($internet -and -not $debeRenovar) {\n        $restante   = [int]($intervaloKeepAlive - $segsDesde)\n        $ssidActual = Get-SSIDActual\n        if ($veniaDeCaida) {\n            $veniaDeCaida = $false\n            Send-Event -type \"ok\" `\n                -msg \"Conexion restaurada -- proxima renovacion en ${restante}s\" `\n                -online $true -restante $restante -reconexion $true `\n                -ssid (if ($ssidActual) { $ssidActual } else { $ssidPreferido })\n            if ($hotspotAlInicio) {\n                Start-Sleep -Seconds 1\n                & netsh wlan start hostednetwork 2>$null | Out-Null\n                Send-Event -type \"info\" -msg \"Hotspot reactivado\"\n            }\n        } else {\n            Send-Event -type \"ok\" `\n                -msg \"Online -- proxima renovacion en ${restante}s\" `\n                -online $true -restante $restante `\n                -ssid (if ($ssidActual) { $ssidActual } else { $ssidPreferido })\n        }\n        Sleep-Reintento -segundos ([math]::Min([double]$restante, 30.0))\n        continue\n    }\n\n    # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\n    # CASO 2 \u2014 Sin internet (v23: if -not $internet)\n    # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\n    if (-not $internet) {\n        # Primera vez: registrar inicio de caida y notificar\n        if (-not $inicioCaida) {\n            $inicioCaida  = [DateTime]::Now\n            $veniaDeCaida = $true\n            # v23: Write-Host \"[!] Sin internet...\"\n            Send-Event -type \"err\" -msg \"Sin internet -- re-autenticando...\"\n        }\n\n        $segsSinInternet = ([DateTime]::Now - $inicioCaida).TotalSeconds\n\n        # Nivel 3: reiniciar NIC \u2014 una sola vez por caida\n        if ((-not $nicReiniciada) -and ($segsSinInternet -ge $segsParaResetNic)) {\n            $nicReiniciada = $true\n            Reset-Adaptador\n            Sleep-Reintento -segundos $intervaloReintento\n            continue\n        }\n\n        # Nivel 2: reconectar WiFi \u2014 una sola vez, antes de la NIC\n        if ((-not $wifiReconectado) -and (-not $nicReiniciada) -and ($segsSinInternet -ge $segsParaReconWifi)) {\n            $wifiReconectado = $true\n            Send-Event -type \"warn\" -msg \"${segsParaReconWifi}s sin internet -- reconectando WiFi a '$ssidPreferido'...\"\n            if ($ssidPreferido -ne \"\") {\n                & netsh wlan connect name=\"$ssidPreferido\" 2>$null | Out-Null\n            }\n            if ($fallbackSinDns) { Start-Sleep -Milliseconds 800; Set-DnsReforzado | Out-Null }\n            Sleep-Reintento -segundos $intervaloReintento\n            continue\n        }\n\n    } else {\n        # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\n        # CASO 3 \u2014 Hay que llamar al portal: renovacion preventiva o reintento\n        # tras fallo (DNS / portal); no spamear \"Renovando\" en cada ciclo de reintento.\n        # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\n        Send-Event -type \"info\" -msg \"Renovando sesion preventivamente...\"\n    }\n\n    # \u2500\u2500 Obtener IP (v23: $ip = Obtener-IP-Portal) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n    $ip = Obtener-IP-Portal\n    if (-not $ip) {\n        # FIX CRITICO: netsh wlan connect NO debe llamarse en cada ciclo\n        # del bucle. Con reintento bajo (0.1-1s) esto se convertia en un\n        # spam de comandos de reconexion varias veces por segundo, lo\n        # cual: (1) interrumpe la asociacion WiFi antes de que termine,\n        # dejandola atascada en \"Conectando...\" indefinidamente, y\n        # (2) puede activar proteccion anti-flood en el propio AP,\n        # haciendo que ese punto de acceso especifico deje de responder\n        # bien a nuestras solicitudes de asociacion por un rato.\n        # Cooldown minimo de 3s ENTRE llamadas reales a netsh, sin\n        # importar que tan rapido este configurado el reintento.\n        $segsDesdeConnectIntento = ([DateTime]::Now - $ultimoIntentoWifiConnect).TotalSeconds\n        $cooldownActivo = if ($cooldownWifiConnect) { [double]$duracionCooldownWifi } else { 0.0 }\n        if ($segsDesdeConnectIntento -ge $cooldownActivo) {\n            $ultimoIntentoWifiConnect = [DateTime]::Now\n            Send-Event -type \"warn\" -msg \"Sin IP 10.x -- reconectando WiFi a '$ssidPreferido'...\"\n            if ($ssidPreferido -ne \"\") {\n                & netsh wlan connect name=\"$ssidPreferido\" 2>$null | Out-Null\n            }\n        } else {\n            Send-Event -type \"gray\" -msg \"Sin IP 10.x -- esperando asociacion WiFi en curso...\"\n        }\n        Sleep-Reintento -segundos $intervaloReintento\n        continue\n    }\n\n    # v23: Write-Host \"[i] IP: $ip\"  (gris, silencioso)\n    Send-Event -type \"gray\" -msg \"IP detectada: $ip\" -ip $ip\n\n    # \u2500\u2500 Autenticar (v23: try { $status = Autenticar $ip }) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n    try {\n        $status     = Autenticar $ip\n        $ssidActual = Get-SSIDActual\n        $fueCalda   = $veniaDeCaida   # capturar ANTES de limpiar\n\n        # Exito: limpiar todo\n        $ultimaRenovacion       = [DateTime]::Now\n        $ultimaReconexionActiva = [DateTime]::Now\n        $inicioCaida      = $null\n        $wifiReconectado  = $false\n        $nicReiniciada    = $false\n\n        if ($fueCalda) {\n            # Venia de caida real \u2014 reconexion exitosa por auth\n            $veniaDeCaida = $false\n            # v23: Write-Host \"[\u2714] Autenticado...\" + \"[\ud83d\udce1] Activando hotspot...\"\n            Send-Event -type \"ok\" `\n                -msg \"Autenticado (reconexion) -- HTTP $status\" `\n                -online $true -httpStatus $status -ip $ip -reconexion $true `\n                -ssid (if ($ssidActual) { $ssidActual } else { $ssidPreferido })\n\n            if ($hotspotAlInicio) {\n                Start-Sleep -Seconds 1\n                & netsh wlan start hostednetwork 2>$null | Out-Null\n                Send-Event -type \"info\" -msg \"Hotspot reactivado\"\n            }\n        } else {\n            # Renovacion preventiva exitosa\n            # v23: Write-Host \"[\u2714] Autenticado...\"\n            Send-Event -type \"ok\" `\n                -msg \"Autenticado -- HTTP $status\" `\n                -online $true -httpStatus $status -ip $ip `\n                -ssid (if ($ssidActual) { $ssidActual } else { $ssidPreferido })\n        }\n\n    } catch {\n        $errMsg = $_.Exception.Message -replace '\"', \"'\"\n\n        # Deteccion especifica de fallo de DNS: aplicar recuperacion rapida\n        # (flush + DNS publico) en vez de esperar toda la escalada normal\n        # (reconectar WiFi a los 45s, reiniciar adaptador a los 70s), que\n        # es lenta y no soluciona un problema que es puramente de DNS.\n        if ($fallbackSinDns -and ($errMsg -match \"could not be resolved|No such host|nombre remoto no se pudo resolver\")) {\n            Invoke-RecuperacionDnsRapida\n        }\n\n        # v23: Write-Host \"[\u2717] Error:...\" sleep; continue\n        # GUI: clasificar por $internet en ESTE ciclo, no por $veniaDeCaida.\n        # Motivo: tras una caida, si ya hay internet pero toca renovar (CASO 3),\n        # $veniaDeCaida puede seguir true hasta auth OK \u2014 entonces el log decia\n        # \"Renovando...\" y luego \"Error auth:\" como si fuera caida (incoherente).\n        if ($internet) {\n            Send-Event -type \"gray\" -msg \"Error auth (preventivo): $errMsg\"\n        } else {\n            Send-Event -type \"warn\" -msg \"Error auth: $errMsg\"\n        }\n\n        Sleep-Reintento -segundos $intervaloReintento\n        continue\n    }\n\n    # FIX: antes era \"Start-Sleep -Seconds 5\" fijo, ignoraba el keep-alive\n    # configurado por el usuario. Ahora respeta $intervaloReintento, que es\n    # lo que permite reconexiones/renovaciones tan rapidas como 0.1s.\n    Sleep-Reintento -segundos $intervaloReintento\n}\n";
    ...
    // El candado se libera apenas el proceso quedo lanzado (o fallo al lanzar),
    // NO durante toda su vida — asi el usuario puede volver a Detener normalmente.
    isTransitioning = false;
  }
}

// stopScript (sincrono, para el IPC 'stop-script'): mata por PID + limpieza
// por nombre de comando como respaldo, y libera el candado siempre.
function stopScript() {
  if (isTransitioning && !psProcess) return; // ya se esta deteniendo/limpio
  isTransitioning = true;
  try {
    if (psProcess) {
    ...
    // Respaldo: matar cualquier powershell.exe cuyo command-line contenga
    // nuestro token de ejecucion actual (isp_xxxxx.ps1), por si el PID
    // trackeado no coincide con el proceso real (p.ej. tras un crash/relanzamiento).
    if (currentRunToken) {
      try {
        const esc = currentRunToken.replace(/'/g, "''");
        execSync(
      ...
      } catch (_) { /* sin huerfanos o comando no disponible — ok */ }
      currentRunToken = null;
    }
  } finally {
    isTransitioning = false;
...
// Version async usada internamente por startScript() antes de relanzar.
function stopScriptAsync() {
...
// Limpieza de seguridad al iniciar: mata cualquier proceso powershell.exe
// huerfano de una ejecucion anterior de InternetSOSplay (prefijo isp_*.ps1)
// que haya quedado vivo por cualquier motivo (crash, cierre forzado de la app, etc).
function killOrphanedScripts() {
  return new Promise((resolve) => {
    try {
      execSync(
        `powershell.exe -NoProfile -NonInteractive -Command ` +
        `"Get-CimInstance Win32_Process -Filter \"Name='powershell.exe'\" | ` +
        `Where-Object { $_.CommandLine -like '*isp_*.ps1*' } | ` +
        `ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"`,
        { windowsHide: true, stdio: 'ignore', timeout: 5000 }
      );
    } catch (_) { /* sin huerfanos — ok */ }
    resolve();
  });
}
...
ipcMain.handle('start-script', async () => {
  if (app.isPackaged) {
    const st = computeLicenseGate();
    if (st.needsLicense) {
  ...
  if (isTransitioning) {
    return { ok: false, error: 'Ya hay una operacion en curso, espera un momento' };
  }
  await startScript();
  return { ok: true };
});
...
// ── AUTO-CAPTURA DE PARÁMETROS DEL PORTAL CAUTIVO ──────────────────
// Hace petición HTTP a neverssl.com (HTTP puro, sin SSL) y captura
// el redirect 302 que envía el AP con los parámetros del portal.
// Funciona aunque el portal no se abra automáticamente en el navegador.
ipcMain.handle('capture-portal-params', async () => {
  return new Promise((resolve) => {
    const http  = require('http');
    const https = require('https');

    // Lista de URLs HTTP puras que el AP debería interceptar y redirigir
    const probeUrls = [
      'http://neverssl.com/',
    ...
    function tryUrl(url) {
      ...
      try {
        const req = http.get(url, {
          ...
          // El AP redirige con 302/301 a la URL del portal cautivo
          if ((status === 301 || status === 302 || status === 303) && location) {
            try {
              // Intentar parsear la URL de redirección
              const redirectUrl = location.startsWith('http') ? location : 'https://' + location;
              const u = new URL(redirectUrl);
              const p = u.searchParams;

              // Buscar parámetros directos
              let acc = p.get('accname') || '';
              let mac = p.get('wlanparameter') || '';
              let ip  = p.get('wlanuserip') || '';
              let tipo = p.get('type') || '';

              // A veces vienen codificados en el path o como parte de otro param
              if (!acc || !mac) {
                // Buscar en toda la URL decodificada
                const decoded = decodeURIComponent(redirectUrl);
                const mAcc = decoded.match(/accname=([^&\s]+)/);
                const mMac = decoded.match(/wlanparameter=([^&\s]+)/);
                const mIp  = decoded.match(/wlanuserip=([^&\s]+)/);
                const mTipo = decoded.match(/[?&T=]([A-Z]+)/);
                if (mAcc) acc = mAcc[1];
                if (mMac) mac = mMac[1];
                if (mIp)  ip  = mIp[1];
              }
            ...
            } catch (parseErr) { /* continuar */ }
          }

          // Sin parámetros útiles en este URL, intentar siguiente
          if (attempted < probeUrls.length) {
            tryUrl(probeUrls[attempted]);
          } else {
            resolve({ ok: false, error: 'No se detectó redirección del portal cautivo. Asegúrate de estar conectado a la red WiFi sin internet activo.' });
          }
        });
        ...
        req.on('error', () => {
          if (attempted < probeUrls.length) {
            ...
            resolve({ ok: false, error: 'No se pudo conectar. Verifica que estés conectado a la red WiFi de Totalplay.' });
          }
        });
        ...
        req.on('timeout', () => {
          ...
          if (attempted < probeUrls.length) {
            ...
            resolve({ ok: false, error: 'Tiempo de espera agotado. El AP no respondió al sondeo.' });
          }
        });
